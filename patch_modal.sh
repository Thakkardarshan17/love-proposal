...
# Find the start and end line of the Content div
START=$(grep -n "{/\* Content \*/}" src/components/DateIdeasModal.tsx | cut -d: -f1)
END=$(grep -n "</div>" src/components/DateIdeasModal.tsx | tail -n 3 | head -n 1 | cut -d: -f1)
END=$((END - 2))

sed -i "${START},${END}c\\$(cat modal_content.txt)" src/components/DateIdeasModal.tsx
